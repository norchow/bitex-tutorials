export const SET_ORIGIN_COUNTRY = 'SET_ORIGIN_COUNTRY'
export const SET_DESTINATION_COUNTRY = 'SET_DESTINATION_COUNTRY'
export const SET_BID_PRICE = 'SET_BID_PRICE'
export const SET_ASK_PRICE = 'SET_ASK_PRICE'
export const CHANGE_SENDER = 'CHANGE_SENDER'
export const CHANGE_ORIGIN_AMOUNT = 'CHANGE_ORIGIN_AMOUNT'
export const CHANGE_DESTINATION_AMOUNT = 'CHANGE_DESTINATION_AMOUNT'
export const CHANGE_PREFUNDED = 'CHANGE_PREFUNDED'
export const CHANGE_RECEIVER = 'CHANGE_RECEIVER'
